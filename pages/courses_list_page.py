...
class CoursesListPage(BasePage):
    ...
    def check_visible_course_card(self, params: CheckVisibleCourseCardParams):
        expect(self.course_preview_image.nth(params.index)).to_be_visible()
        ...
        expect(self.course_max_text.nth(params.index)).to_contain_text(f'Max score: {params.max_score}')

        expect(self.course_min_text.nth(params.index)).to_be_visible()
        expect(self.course_min_text.nth(params.index)).to_contain_text(f'Min score: {params.min_score}')

        expect(self.course_estimated_text.nth(params.index)).to_be_visible()
        expect(self.course_estimated_text.nth(params.index)).to_have_text(f'Estimated time: {params.estimated_time}')
